...
export function alignBlock({ block, text, words }: { block: Descendant; text: any; words: TranscriptWord[] }): Element {
...
function updateBlocksTimestamps(slateJsValue: Descendant[]): Element[] {
  return slateJsValue.map((block) => {
    return updateIndividualBlockTimestamps(block);
  });
}

export default updateBlocksTimestamps;
